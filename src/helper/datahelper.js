import { ICON_HELPER } from "./iconhelper";
import { IMAGE_HELPER } from "./imagehelper";

export const top_nav = [
  {
    id: "1",
    icon: ICON_HELPER.MAIL_ICON,
    heading: "Send Email",
    content: "vijayan@sriarasutex.in",
  },
  {
    id: "2",
    icon: ICON_HELPER.CLOCK_ICON,
    heading: "Working Hours",
    content: "Mon-Sat 09:00 am to 07:00 pm",
  },
  {
    id: "3",
    icon: ICON_HELPER.phone_icon,
    heading: "Have any Questions?",
    content: "0091-4324-233551",
  },
];

export const bottom_menu = [
  {
    id: "1",
    name: "Home",
    link: "/",
  },
  {
    id: "2",
    name: "Know Us",
    link: "/aboutus",
  },
  {
    id: "3",
    name: "Product Vault",
    link: "/collections",
  },
  {
    id: "4",
    name: "Articles",
    link: "/blogs",
  },
  {
    id: "5",
    name: "Reach Us",
    link: "/contact",
  },
];

export const nav_links = [
  {
    id: "1",
    icon: ICON_HELPER.FACEBOOK_ICON,
    name: "Facebook",
    link: "https://www.facebook.com/profile.php?id=100069418375129",
  },
  {
    id: "2",
    icon: ICON_HELPER.TWITTER_ICON,
    name: "Twitter",
    link: "https://x.com/SriArasuTex",
  },
  {
    id: "3",
    icon: ICON_HELPER.INSTA_ICON,
    name: "Instagram",
    link: "https://www.instagram.com/sriarasutex_/",
  },
  {
    id: "4",
    icon: ICON_HELPER.PRINTEREST_ICON,
    name: "Printerest",
    link: "https://dk.pinterest.com/Arasu_/",
  },
];

export const footer_collection = [
  {
    id: "1",
    name: "Kitchen Set",
  },
  {
    id: "2",
    name: "Living Room",
  },
  {
    id: "3",
    name: "Table Lines",
  },
  {
    id: "4",
    name: "Beddings",
  },
];

export const about = {
  title: "ABOUT US",
  description: "We have 36+ years experience. Sri Arasu Tex has got exclusive manfacturing home textile",
  socials: [
    {
      id: "1",
      icon: ICON_HELPER.FACEBOOK_ICON,
      name: "Facebook",
      link: "https://www.facebook.com/profile.php?id=100069418375129",
    },
    {
      id: "2",
      icon: ICON_HELPER.TWITTER_ICON,
      name: "Twitter",
      link: "https://x.com/SriArasuTex",
    },
    {
      id: "3",
      icon: ICON_HELPER.INSTA_ICON,
      name: "Instagram",
      link: "https://www.instagram.com/sriarasutex_/",
    },
    {
      id: "4",
      icon: ICON_HELPER.PRINTEREST_ICON,
      name: "Printerest",
      link: "https://dk.pinterest.com/Arasu_/",
    },
  ],
};

export const hero_about = [
  {
    id: "1",
    icon: ICON_HELPER.COUNTRY_ICON,
    heading: "Countries Exporting",
    content: "Europe & USA",
  },
  {
    id: "2",
    icon: ICON_HELPER.QUALITY_ICON,
    heading: "Qualities Dealing",
    content: "Recycled Cotton & Polyester",
  },
];

export const home_collections = [
  {
    id: "1",
    name: "Kitchen Set",
    image: IMAGE_HELPER.HOME_COLLECTION7,
  },
  {
    id: "2",
    name: "Living Room",
    image: IMAGE_HELPER.HOME_COLLECTION1,
  },
  {
    id: "3",
    name: "Table Lines",
    image: IMAGE_HELPER.HOME_COLLECTION2,
  },
  {
    id: "4",
    name: "Beddings",
    image: IMAGE_HELPER.HOME_COLLECTION3,
  },
  {
    id: "5",
    name: "Baby",
    image: IMAGE_HELPER.HOME_COLLECTION4,
  },
  {
    id: "6",
    name: "Bath",
    image: IMAGE_HELPER.HOME_COLLECTION5,
  },
  {
    id: "7",
    name: "Outdoor",
    image: IMAGE_HELPER.HOME_COLLECTION6,
  },
];

export const servicesData = [
  {
    title: "Fabric Sourcing",
    description: "We offer high-quality, sustainable fabrics from trusted global suppliers.",
    icon: IMAGE_HELPER.HOME_PROCESS1,
  },
  {
    title: "Custom Textile Design",
    description: "Get exclusive patterns and prints tailored to your brand identity.",
    icon: IMAGE_HELPER.HOME_PROCESS2,
  },
  {
    title: "Manufacturing & Delivery",
    description: "Efficient production with on-time delivery and strict quality assurance.",
    icon: IMAGE_HELPER.HOME_PROCESS3,
  },
  {
    title: "Quality Control & Support",
    description: "Comprehensive quality checks and ongoing support to ensure client satisfaction.",
    icon: IMAGE_HELPER.HOME_PROCESS4,
  },
];

// helpers/blogData.js
export const blogPosts = [
  {
    id: 1,
    date: "JUN 20, 2024",
    author: "Admin",
    comments: 13,
    title: "Textilegence magazine and advanced stage.",
    image: IMAGE_HELPER.BLOG_1,
  },
  {
    id: 2,
    date: "MAY 12, 2024",
    author: "Admin",
    comments: 8,
    title: "Innovations in modern fabric processing.",
    image: IMAGE_HELPER.BLOG_2,
  },
  {
    id: 3,
    date: "APR 02, 2024",
    author: "Admin",
    comments: 5,
    title: "Designing fashion for the future.",
    image: IMAGE_HELPER.BLOG_3,
  },
  {
    id: 1,
    date: "JUN 20, 2024",
    author: "Admin",
    comments: 13,
    title: "Textilegence magazine and advanced stage.",
    image: IMAGE_HELPER.BLOG_1,
  },
  {
    id: 2,
    date: "MAY 12, 2024",
    author: "Admin",
    comments: 8,
    title: "Innovations in modern fabric processing.",
    image: IMAGE_HELPER.BLOG_2,
  },
  {
    id: 3,
    date: "APR 02, 2024",
    author: "Admin",
    comments: 5,
    title: "Designing fashion for the future.",
    image: IMAGE_HELPER.BLOG_3,
  },
];

export const PRODUCT_COLLECTIONS_CATEGORIES = [
  {
    id: "1",
    name: "Kitchen Set",
    category_name: "kitchen_set",
    imageurl: IMAGE_HELPER.HOME_COLLECTION7,
    heroimage: IMAGE_HELPER.GLOVE1_IMAGE,
    content: "Upgrade your kitchen with our premium 5-piece kitchen set designed for both style and function.This set includes a durable apron, a soft kitchen towel, a heat-resistant oven glove, a matching pot holder, and a protective oven cover.Crafted from high-quality, breathable cotton fabric for comfort and long-lasting use.The apron provides full coverage with adjustable straps for a perfect fit.The glove and pot holder are padded to protect your hands from hot pans and trays.Our kitchen towel is ultra-absorbent, perfect for quick drying and cleaning.The oven cover adds a neat and decorative touch to your appliance.Each piece is coordinated in color and pattern to enhance your kitchen décor.Ideal for everyday cooking, baking, and entertaining guests.A thoughtful gift choice for home cooks, bakers, or new homeowners.",
  },
  {
    id: "2",
    name: "Living Room",
    category_name: "living_room",
    imageurl: IMAGE_HELPER.HOME_COLLECTION1,
    heroimage: IMAGE_HELPER.HOME_COLLECTION1,
    content: "Transform your living space with our elegant living room set that blends comfort and style.The set includes premium curtains, soft cushions, cozy throws, large floor cushions, and a sturdy door stopper.Our curtains add privacy and sophistication while gently filtering natural light.Plush cushions and throws bring warmth and texture, perfect for relaxing or entertaining.Fat cushions offer casual seating and a cozy vibe for movie nights or guests.Crafted from high-quality fabrics for lasting softness and durability.Thoughtfully coordinated colors and patterns enhance any interior theme.The door stopper adds a functional yet stylish touch to keep your space draft-free.Every piece is designed to create a welcoming, cozy, and stylish atmosphere.Ideal for modern homes that value comfort with a designer look.",
  },
  {
    id: "3",
    name: "Table Lines",
    category_name: "table_line",
    imageurl: IMAGE_HELPER.HOME_COLLECTION2,
    heroimage: IMAGE_HELPER.OUT_TABLE2,
    content: "Elevate your dining experience with our beautifully crafted table linen set.This collection includes tablecloths, runners, placemats, and coordinating napkins.Made from premium fabrics that are both durable and easy to maintain.Designed to protect your table while adding a touch of elegance and charm.Our linens feature refined textures and timeless prints for any occasion.Perfect for daily meals, festive gatherings, or formal dinners.The table runner adds a stylish centerpiece while placemats keep things neat.Soft napkins complete the look with both function and finesse.Available in a range of colors and styles to suit any décor.Turn every meal into a special occasion with our table linen collection.",
  },
  {
    id: "4",
    name: "Beddings",
    category_name: "bedding",
    imageurl: IMAGE_HELPER.HOME_COLLECTION3,
    heroimage: IMAGE_HELPER.B_DEUVET1,
    content: "Create the perfect sleep sanctuary with our luxurious bedding collection.This set includes a flat bed cover, fitted bed cover, plush duvet, matching duvet cover, soft pillows, and pillow covers.Crafted from premium fabrics for unmatched comfort and breathability.The fitted bed cover ensures a snug fit, while the flat cover adds a layered finish.Our fluffy duvet offers warmth without the weight, ideal for all seasons.The duvet cover brings style and easy maintenance to your bedding.Pillows provide the right balance of softness and support for restful sleep.Coordinated pillow covers add a polished and elegant look to your bed.Available in a variety of textures, colors, and patterns to suit any décor.Enjoy hotel-style comfort and design in the comfort of your own home.",
  },
  {
    id: "5",
    name: "Bath",
    category_name: "bath",
    imageurl: IMAGE_HELPER.HOME_COLLECTION4,
    heroimage: IMAGE_HELPER.BATH_CU1,
    content: "Indulge in spa-like comfort at home with our premium bath collection.This set includes soft towels, a cozy bathrobe, non-slip bath footwear, and stylish shower curtains.Made from highly absorbent, quick-drying fabrics for everyday luxury.Our plush towels are gentle on the skin and perfect for daily use.The bathrobe offers warmth and relaxation after every shower or bath.Durable and water-resistant, the shower curtains add both privacy and elegance.Breathable and lightweight bath footwear ensures safety and comfort.Designed with coordinated colors to enhance your bathroom décor.Each piece combines practicality with a touch of refined style.Perfect for creating a calm, refreshing, and organized bath space.",
  },
  {
    id: "6",
    name: "Outdoor",
    category_name: "outdoor",
    imageurl: IMAGE_HELPER.HOME_COLLECTION5,
    heroimage: IMAGE_HELPER.OUT_TABLE3,
    content: "Enhance your outdoor living with our stylish and functional outdoor essentials.This collection features a durable table cloth, comfortable chair pads, and casual caps for sunny days.Our outdoor table cloth is weather-resistant and easy to clean, perfect for patios and picnics.Chair pads offer extra comfort and support, ideal for long lounging sessions.Each piece is made with fade-resistant fabric to withstand the elements.The caps provide sun protection while adding a sporty touch to your outdoor look.Designed to complement any garden, balcony, or outdoor seating area.Coordinated colors and patterns bring a refreshing and vibrant vibe.Perfect for weekend brunches, family gatherings, or quiet evenings outside.Make every outdoor moment comfortable, stylish, and sun-safe with our collection.",
  },
  {
    id: "7",
    name: "Baby",
    category_name: "baby",
    imageurl: IMAGE_HELPER.HOME_COLLECTION6,
    heroimage: IMAGE_HELPER.BABY_BED1,
    content: "Wrap your little one in comfort with our thoughtfully designed baby essentials collection.This set includes soft baby pillows, gentle towels, cozy throws, play mats, a snug sleeping bed, and cute bibs.Crafted from ultra-soft, hypoallergenic materials safe for delicate baby skin.Our baby pillows support proper posture and restful naps.The absorbent towels and bibs make cleanup easy during bath and mealtime.Plush throws offer warmth and comfort for cuddles and naps.The sleeping bed ensures a secure and peaceful sleep environment.Colorful, cushioned play mats encourage safe play and tummy time.Adorably designed with soft pastels and playful prints to delight both babies and parents.Perfect for gifting or daily use, this collection combines comfort, safety, and charm.",
  },
];

export const PRODUCT_COLLECTIONS_SUB_CATEGORIES = [
  //kitchen
  {
    id: 1,
    category_id: "kitchen_set",
    name: "Glove",
    sub_category_id: "glove",
  },
  {
    id: 2,
    category_id: "kitchen_set",
    name: "Pot Holder",
    sub_category_id: "pot_holder",
  },
  {
    id: 3,
    category_id: "kitchen_set",
    name: "Kitchen Towel",
    sub_category_id: "kitchen_towel",
  },
  {
    id: 4,
    category_id: "kitchen_set",
    name: "Oven Cover",
    sub_category_id: "oven_cover",
  },
  {
    id: 5,
    category_id: "kitchen_set",
    name: "Apron",
    sub_category_id: "apron",
  },
  {
    id: 6,
    category_id: "kitchen_set",
    name: "Fridge Handle cover",
    sub_category_id: "fridge_cover",
  },

  //living Room
  {
    id: 7,
    category_id: "living_room",
    name: "Curtains",
    sub_category_id: "curtains",
  },
  {
    id: 8,
    category_id: "living_room",
    name: "Cushions",
    sub_category_id: "cushions",
  },
  {
    id: 9,
    category_id: "living_room",
    name: "Throws",
    sub_category_id: "throws",
  },
  {
    id: 11,
    category_id: "living_room",
    name: "Fat Cushions",
    sub_category_id: "fat_cushions",
  },
  {
    id: 12,
    category_id: "living_room",
    name: "Door Stopper",
    sub_category_id: "door",
  },

  //Table Lines
  {
    id: 13,
    category_id: "table_line",
    name: "Table Cloth",
    sub_category_id: "table_cloth",
  },
  {
    id: 14,
    category_id: "table_line",
    name: "Runner",
    sub_category_id: "runner",
  },
  {
    id: 15,
    category_id: "table_line",
    name: "Napkins",
    sub_category_id: "napkins",
  },
  {
    id: 16,
    category_id: "table_line",
    name: "Bread Basket",
    sub_category_id: "bread_basket",
  },
  {
    id: 17,
    category_id: "table_line",
    name: "Egg Holder",
    sub_category_id: "egg_holder",
  },
  {
    id: 18,
    category_id: "table_line",
    name: "coaster",
    sub_category_id: "coaster",
  },
  {
    id: 19,
    category_id: "table_line",
    name: "Chairpad",
    sub_category_id: "chairpad",
  },
  {
    id: 20,
    category_id: "table_line",
    name: "Chair Cover",
    sub_category_id: "chari_cover",
  },
  //bedding
  {
    id: 21,
    category_id: "bedding",
    name: "Flat Bed Cover",
    sub_category_id: "flat_bed",
  },
  {
    id: 22,
    category_id: "bedding",
    name: "Fitted Bed Cover",
    sub_category_id: "fitted_bed",
  },
  {
    id: 23,
    category_id: "bedding",
    name: "Duvet",
    sub_category_id: "duvet",
  },
  {
    id: 24,
    category_id: "bedding",
    name: "Duvet Cover",
    sub_category_id: "duvet_cover",
  },
  {
    id: 25,
    category_id: "bedding",
    name: "Pillows",
    sub_category_id: "pillows",
  },
  {
    id: 26,
    category_id: "bedding",
    name: "Pillows Cover",
    sub_category_id: "pillow_cover",
  },
  // {
  //   id: 27,
  //   category_id: "bedding",
  //   name: "Bed Cushions",
  //   sub_category_id: "bed_cushions",
  // },

  //bath
  {
    id: 28,
    category_id: "bath",
    name: "Bath Towels ",
    sub_category_id: "bath_towels",
  },
  {
    id: 29,
    category_id: "bath",
    name: "Bath Rope",
    sub_category_id: "bath_rope",
  },
  {
    id: 30,
    category_id: "bath",
    name: "Bath Curtains",
    sub_category_id: "bath_curtains",
  },
  {
    id: 31,
    category_id: "bath",
    name: "Bath Footwear",
    sub_category_id: "bath_footwear",
  },

  //outdoor
  {
    id: 32,
    category_id: "outdoor",
    name: "Outdoor Table Cloth",
    sub_category_id: "outdoor_table_cloth",
  },
  {
    id: 33,
    category_id: "outdoor",
    name: "Chairpad",
    sub_category_id: "outdoor_chairpad",
  },
  {
    id: 34,
    category_id: "outdoor",
    name: "Bags",
    sub_category_id: "bags",
  },
  // {
  //   id: 35,
  //   category_id: "outdoor",
  //   name: "Beach Wears",
  //   sub_category_id: "beach_wears",
  // },
  {
    id: 36,
    category_id: "outdoor",
    name: "Caps(cotton)",
    sub_category_id: "caps",
  },

  //baby
  {
    id: 37,
    category_id: "baby",
    name: "Towels",
    sub_category_id: "baby_towels",
  },
  {
    id: 38,
    category_id: "baby",
    name: "Pillows",
    sub_category_id: "baby_pillows",
  },
  {
    id: 39,
    category_id: "baby",
    name: "Play Mats",
    sub_category_id: "play_mats",
  },
  {
    id: 40,
    category_id: "baby",
    name: "Sleeping Bed",
    sub_category_id: "sleeping_bed",
  },
  {
    id: 41,
    category_id: "baby",
    name: "Throws",
    sub_category_id: "baby_throws",
  },
  {
    id: 42,
    category_id: "baby",
    name: "Bib",
    sub_category_id: "bib",
  },
  // {
  //   id: 43,
  //   category_id: "baby",
  //   name: "Nursing Pillow",
  //   sub_category_id: "nursing_pillow",
  // },
];

export const PRODUCTS = [
  {
    id: 1,
    sub_category_id: "glove",
    images: [IMAGE_HELPER.GLOVE1_IMAGE, IMAGE_HELPER.GLOVE2_IMAGE, IMAGE_HELPER.GLOVE3_IMAGE, IMAGE_HELPER.GLOVE4_IMAGE],
    description: "High-quality kitchen gloves.\nDurable and heat-resistant.\nPerfect for everyday use.\nOffers firm grip and comfort.\nMachine washable and reusable.",
  },
  {
    id: 2,
    sub_category_id: "pot_holder",
    images: [IMAGE_HELPER.POT1, IMAGE_HELPER.POT2, IMAGE_HELPER.POT3, IMAGE_HELPER.POT4],
    description: "Protect your hands from hot pots.\nMade with premium cotton.\nStylish and functional design.\nPerfect for kitchen use.\nResistant to wear and tear.",
  },
  {
    id: 3,
    sub_category_id: "kitchen_towel",
    images: [IMAGE_HELPER.KTOWEL1, IMAGE_HELPER.KTOWEL2, IMAGE_HELPER.KTOWEL3, IMAGE_HELPER.KTOWEL4],
    description: "Soft and absorbent kitchen towels.\nIdeal for cleaning and drying.\nMachine washable for convenience.\nFade-resistant colors.\nPerfect for daily kitchen use.",
  },
  {
    id: 4,
    sub_category_id: "apron",
    images: [IMAGE_HELPER.APRON1, IMAGE_HELPER.APRON2, IMAGE_HELPER.APRON3, IMAGE_HELPER.APRON4],
    description: "Stylish aprons for home chefs.\nAdjustable fit and deep pockets.\nKeeps your clothes clean while cooking.\nDurable and breathable fabric.\nAvailable in modern prints.",
  },
  {
    id: 5,
    sub_category_id: "oven_cover",
    images: [IMAGE_HELPER.OVEN1, IMAGE_HELPER.oven2, IMAGE_HELPER.oven3, IMAGE_HELPER.oven4],
    description: "Elegant oven covers.\nProtects and decorates.\nFits most standard ovens.\nEasy to clean material.\nAdds style to your kitchen.",
  },
  {
    id: 6,
    sub_category_id: "fridge_cover",
    images: [IMAGE_HELPER.HANDLE_1, IMAGE_HELPER.HANDLE_2, IMAGE_HELPER.HANDLE_3, IMAGE_HELPER.HANDLE_4],
    description: "Fridge handle covers.\nPrevents smudges and stains.\nSoft and durable fabric.\nFits most fridge handles.\nEasy to attach and remove.",
  },
  {
    id: 7,
    sub_category_id: "curtains",
    images: [IMAGE_HELPER.LVCUR1, IMAGE_HELPER.LVCUR2, IMAGE_HELPER.LVCUR3, IMAGE_HELPER.LVCUR4],
    description: "Elegant window curtains.\nBlocks light effectively.\nEnhances room decor.\nEasy to hang and clean.\nAvailable in various designs.",
  },
  {
    id: 8,
    sub_category_id: "cushions",
    images: [IMAGE_HELPER.LVCU1, IMAGE_HELPER.LVCU2, IMAGE_HELPER.LVCU3, IMAGE_HELPER.LVCU4],
    description: "Soft and plush cushions.\nAdds comfort and style.\nGreat for sofa or bed.\nRemovable and washable cover.\nMultiple color options.",
  },
  {
    id: 9,
    sub_category_id: "throws",
    images: [IMAGE_HELPER.THROWS1, IMAGE_HELPER.THROWS2, IMAGE_HELPER.THROWS3, IMAGE_HELPER.THROWS4],
    description: "Warm and cozy throws.\nPerfect for snuggling.\nStylish texture and color.\nIdeal for living rooms.\nDurable and soft material.",
  },
  {
    id: 10,
    sub_category_id: "fat_cushions",
    images: [IMAGE_HELPER.LVFAT1, IMAGE_HELPER.LVFAT2, IMAGE_HELPER.LVFAT3, IMAGE_HELPER.LVFAT4],
    description: "Plush fat cushions.\nThick and comfortable.\nPerfect for floor seating.\nAdds a cozy vibe.\nAvailable in trendy colors.",
  },
  {
    id: 11,
    sub_category_id: "door",
    images: [IMAGE_HELPER.DOOR1, IMAGE_HELPER.DOOR2, IMAGE_HELPER.DOOR3, IMAGE_HELPER.DOOR4],
    description: "Decorative door curtains.\nEnhances privacy and style.\nMade from soft fabric.\nEasy to install and wash.\nFits standard door sizes.",
  },
  {
    id: 12,
    sub_category_id: "table_cloth",
    images: [IMAGE_HELPER.TABLE1, IMAGE_HELPER.TABLE2, IMAGE_HELPER.TABLE3, IMAGE_HELPER.TABLE4],
    description: "Elegant table cloths.\nProtects surfaces from stains.\nBeautiful prints and patterns.\nPerfect for everyday use.\nMachine washable fabric.",
  },
  {
    id: 13,
    sub_category_id: "runner",
    images: [IMAGE_HELPER.TRUNNER1, IMAGE_HELPER.TRUNNER2, IMAGE_HELPER.TRUNNER3, IMAGE_HELPER.TRUNNER4],
    description: "Stylish table runners.\nAdds charm to your table.\nPerfect for dining setup.\nAvailable in many colors.\nEasy to wash and reuse.",
  },
  {
    id: 14,
    sub_category_id: "napkins",
    images: [IMAGE_HELPER.NAP1_MAGE, IMAGE_HELPER.NAP2_MAGE, IMAGE_HELPER.NAP3_MAGE, IMAGE_HELPER.NAP4_MAGE],
    description: "Soft cotton napkins.\nReusable and eco-friendly.\nPerfect for meals and events.\nElegant and absorbent.\nSet of four per pack.",
  },
  {
    id: 15,
    sub_category_id: "bread_basket",
    images: [IMAGE_HELPER.BRED_BASKET1, IMAGE_HELPER.BRED_BASKET2, IMAGE_HELPER.BRED_BASKET3, IMAGE_HELPER.BRED_BASKET4],
    description: "Charming bread baskets.\nIdeal for breakfast and snacks.\nSturdy and elegant.\nMade with quality materials.\nEasy to clean and store.",
  },
  {
    id: 16,
    sub_category_id: "egg_holder",
    images: [IMAGE_HELPER.EGG_1, IMAGE_HELPER.EGG_2, IMAGE_HELPER.EGG_3, IMAGE_HELPER.EGG_4],
    description: "Handy egg holders.\nPerfect for serving or storing.\nKeeps eggs safe and neat.\nStylish and compact.\nGreat for any kitchen.",
  },
  {
    id: 17,
    sub_category_id: "coaster",
    images: [IMAGE_HELPER.COASTER1, IMAGE_HELPER.COASTER2, IMAGE_HELPER.COASTER3, IMAGE_HELPER.COASTER4],
    description: "Elegant drink coasters.\nProtects your surfaces.\nNon-slip and absorbent.\nComes in a set of four.\nAdds charm to your decor.",
  },
  {
    id: 18,
    sub_category_id: "chairpad",
    images: [IMAGE_HELPER.CHAIR_PAD1, IMAGE_HELPER.CHAIR_PAD2, IMAGE_HELPER.CHAIR_PAD3, IMAGE_HELPER.CHAIR_PAD4],
    description: "Comfortable chair pads.\nProvides support and softness.\nTies to secure in place.\nPerfect for dining chairs.\nAvailable in many styles.",
  },
  {
    id: 19,
    sub_category_id: "chari_cover",
    images: [IMAGE_HELPER.CHAIR_COVER1, IMAGE_HELPER.CHAIR_COVER2, IMAGE_HELPER.CHAIR_COVER3, IMAGE_HELPER.CHAIR_COVER4],
    description: "Elegant chair covers.\nGives chairs a new look.\nEasy to fit and remove.\nProtects from dust and stains.\nWashable and reusable.",
  },
  {
    id: 20,
    sub_category_id: "flat_bed",
    images: [IMAGE_HELPER.BB_COVER1, IMAGE_HELPER.BB_COVER2, IMAGE_HELPER.BB_COVER3, IMAGE_HELPER.BB_COVER4],
    description: "Soft flat bed sheets.\nSmooth and breathable.\nAvailable in all sizes.\nPerfect for restful sleep.\nEasy to maintain fabric.",
  },
  {
    id: 21,
    sub_category_id: "fitted_bed",
    images: [IMAGE_HELPER.FB_COVER1, IMAGE_HELPER.FB_COVER2, IMAGE_HELPER.FB_COVER3, IMAGE_HELPER.FB_COVER4],
    description: "Stretchable fitted sheets.\nSecure fit for mattress.\nSoft and skin-friendly.\nGreat for daily use.\nColorfast and durable.",
  },
  {
    id: 22,
    sub_category_id: "duvet",
    images: [IMAGE_HELPER.B_DEUVET1, IMAGE_HELPER.B_DEUVET2, IMAGE_HELPER.B_DEUVET3, IMAGE_HELPER.B_DEUVET4],
    description: "Cozy duvets for winter.\nLightweight yet warm.\nFilled with soft fiber.\nElegant outer cover.\nAvailable in all sizes.",
  },
  {
    id: 23,
    sub_category_id: "duvet_cover",
    images: [IMAGE_HELPER.DUVET_COVER1, IMAGE_HELPER.DUVET_COVER2, IMAGE_HELPER.DUVET_COVER3, IMAGE_HELPER.DUVET_COVER4],
    description: "Protective duvet covers.\nEasy to put on and remove.\nMachine washable fabric.\nComes with button/zip closure.\nAvailable in trendy designs.",
  },
  {
    id: 24,
    sub_category_id: "pillows",
    images: [IMAGE_HELPER.B_PILLOWS1, IMAGE_HELPER.B_PILLOWS2, IMAGE_HELPER.B_PILLOWS3, IMAGE_HELPER.B_PILLOWS4],
    description: "Soft and fluffy pillows.\nProvides great neck support.\nRetains shape for long.\nPerfect for sound sleep.\nComes in pairs or singles.",
  },
  {
    id: 25,
    sub_category_id: "pillow_cover",
    images: [IMAGE_HELPER.B_PCOVER1, IMAGE_HELPER.B_PCOVER2, IMAGE_HELPER.B_PCOVER3, IMAGE_HELPER.B_PCOVER4],
    description: "Decorative pillow covers.\nZippered and stylish.\nSoft texture and colorfast.\nFits standard pillows.\nPerfect match for bedding.",
  },
  {
    id: 27,
    sub_category_id: "bath_towels",
    images: [IMAGE_HELPER.BATH_TOWEL1, IMAGE_HELPER.BATH_TOWEL2, IMAGE_HELPER.BATH_TOWEL3, IMAGE_HELPER.BATH_TOWEL4],
    description: "Highly absorbent towels.\nSoft and skin-friendly.\nPerfect for everyday use.\nDries quickly after use.\nComes in multiple colors.",
  },
  {
    id: 28,
    sub_category_id: "bath_rope",
    images: [IMAGE_HELPER.BATH_ROPE1, IMAGE_HELPER.BATH_ROPE2, IMAGE_HELPER.BATH_ROPE3, IMAGE_HELPER.BATH_ROPE4],
    description: "Comfortable bath robes.\nWraps you in warmth.\nSoft and breathable fabric.\nPerfect after shower.\nEasy to care and wash.",
  },
  {
    id: 29,
    sub_category_id: "bath_curtains",
    images: [IMAGE_HELPER.BATH_CU1, IMAGE_HELPER.BATH_CU2, IMAGE_HELPER.BATH_CU3, IMAGE_HELPER.BATH_CU4],
    description: "Waterproof bath curtains.\nProtects from water splashes.\nAdds a fresh look.\nDurable and stylish.\nFits standard shower rods.",
  },
  {
    id: 30,
    sub_category_id: "bath_footwear",
    images: [IMAGE_HELPER.BATH_FOOT1, IMAGE_HELPER.BATH_FOOT2, IMAGE_HELPER.BATH_FOOT3, IMAGE_HELPER.BATH_FOOT4],
    description: "Soft bathroom slippers.\nNon-slip soles for safety.\nLightweight and durable.\nComfortable indoor wear.\nAvailable in various sizes.",
  },
  {
    id: 31,
    sub_category_id: "outdoor_table_cloth",
    images: [IMAGE_HELPER.OUT_TABLE1, IMAGE_HELPER.OUT_TABLE2, IMAGE_HELPER.OUT_TABLE3, IMAGE_HELPER.OUT_TABLE4],
    description: "Outdoor tablecloths.\nPerfect for garden setups.\nResistant to weather.\nAttractive patterns and colors.\nEasy to clean material.",
  },
  {
    id: 32,
    sub_category_id: "outdoor_chairpad",
    images: [IMAGE_HELPER.OUT_CHAIR1, IMAGE_HELPER.OUT_CHAIR2, IMAGE_HELPER.OUT_CHAIR3, IMAGE_HELPER.OUT_CHAIR4],
    description: "Outdoor chair cushions.\nSoft and weatherproof.\nBright and cheerful designs.\nAdds comfort and color.\nIdeal for patio seating.",
  },
  {
    id: 33,
    sub_category_id: "bags",
    images: [IMAGE_HELPER.OUT_BAG1, IMAGE_HELPER.OUT_BAG2, IMAGE_HELPER.OUT_BAG3, IMAGE_HELPER.OUT_BAG4],
    description: "Stylish multipurpose bags.\nPerfect for everyday use.\nStrong and spacious.\nEco-friendly fabric.\nTrendy and long-lasting.",
  },
  {
    id: 35,
    sub_category_id: "caps",
    images: [IMAGE_HELPER.CAPS1, IMAGE_HELPER.CAPS2, IMAGE_HELPER.CAPS3, IMAGE_HELPER.CAPS4],
    description: "Comfortable cotton caps.\nProtects from sunlight.\nAdjustable fit for all.\nLightweight and stylish.\nUnisex and washable.",
  },
  {
    id: 36,
    sub_category_id: "baby_towels",
    images: [IMAGE_HELPER.BABY_TOWEL1, IMAGE_HELPER.BABY_TOWEL2, IMAGE_HELPER.BABY_TOWEL3, IMAGE_HELPER.BABY_TOWEL4],
    description: "Soft baby towels.\nMade with gentle fabric.\nQuick absorbency.\nIdeal for delicate skin.\nAvailable in cute prints.",
  },
  {
    id: 37,
    sub_category_id: "play_mats",
    images: [IMAGE_HELPER.BABY_MATS1, IMAGE_HELPER.BABY_MATS2, IMAGE_HELPER.BABY_MATS3, IMAGE_HELPER.BABY_MATS4],
    description: "Colorful baby play mats.\nSoft and safe surface.\nEncourages baby play.\nLightweight and portable.\nEasy to clean and store.",
  },
  {
    id: 38,
    sub_category_id: "sleeping_bed",
    images: [IMAGE_HELPER.BABY_BED1, IMAGE_HELPER.BABY_BED2, IMAGE_HELPER.BABY_BED3, IMAGE_HELPER.BABY_BED4],
    description: "Comfortable baby beds.\nSoft cushioning for rest.\nPortable and light.\nSafe for sleep and nap.\nWashable and hygienic.",
  },
  {
    id: 39,
    sub_category_id: "baby_throws",
    images: [IMAGE_HELPER.BABY_THROWS1, IMAGE_HELPER.BABY_THROWS2, IMAGE_HELPER.BABY_THROWS3, IMAGE_HELPER.BABY_THROWS4],
    description: "Cute baby throws.\nWarm and gentle fabric.\nPerfect for crib or travel.\nAvailable in soft colors.\nLightweight and cozy.",
  },
  {
    id: 40,
    sub_category_id: "bib",
    images: [IMAGE_HELPER.BABY_BIB1, IMAGE_HELPER.BABY_BIB2, IMAGE_HELPER.BABY_BIB3, IMAGE_HELPER.BABY_BIB4],
    description: "Adorable baby bibs.\nKeeps clothes clean.\nSoft and absorbent fabric.\nSnap or velcro closure.\nFun designs for babies.",
  },
  {
    id: 42,
    sub_category_id: "baby_pillows",
    images: [IMAGE_HELPER.BABY_PILLOW1, IMAGE_HELPER.BABY_PILLOW2, IMAGE_HELPER.BABY_PILLOW3, IMAGE_HELPER.BABY_PILLOW4],
    description: "Gentle baby pillows.\nDesigned for infant head support.\nReduces flat head syndrome.\nMade with safe materials.\nCompact and travel-friendly.",
  },
];

export const GET_SUBCATEGORY_NAME = (value) => {
  try {
    let result = PRODUCT_COLLECTIONS_SUB_CATEGORIES.find((res) => res.sub_category_name === value);
    return result?.name;
  } catch (err) {
    console.log(err);
  }
};

export const GET_SUBCATEGORY_BY_CATEGORY = (value) => {
  try {
    let result = PRODUCT_COLLECTIONS_SUB_CATEGORIES.find((res) => res.category_id === value);

    return result?.sub_category_id;
  } catch (err) {
    console.log(err);
  }
};
export const GET_PRODUCT_IMAGE = (value, custom = false) => {
  try {
    let result = PRODUCTS.find((res) => res.sub_category_id === value);
    if (!result || !result.images.length) return null;

    let index = custom && result.images[1] ? 1 : 0;
    return result.images[index];
  } catch (err) {
    console.log(err);
  }
};

// blogData.js

export const blogMain = {
  image: IMAGE_HELPER.BATH_CU2,
  category: "Analytics",
  date: "Nov 09, 2024",
  comments: 13,
  paragraphs: ["Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s...", "It has survived not only five centuries, but also the leap into electronic typesetting..."],
};

export const recentPosts = [
  {
    date: "January 20, 2024",
    image: "/recent-post-1.jpg",
    title: "The advantages of a digital supply strategy",
  },
  {
    date: "January 20, 2024",
    image: "/recent-post-2.jpg",
    title: "We very careful handling the valuable goods",
  },
  {
    date: "January 20, 2024",
    image: "/recent-post-3.jpg",
    title: "Choosing the best logistics for your business",
  },
];
