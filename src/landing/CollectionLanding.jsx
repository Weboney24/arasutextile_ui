...
const CollectionLanding = () => {
   ...
   const handleWheel = (e) => {
     e.stopPropagation(); // prevent closing modal on scroll
     const zoomIntensity = 0.1;
     if (e.deltaY < 0) {
       setScale((prev) => Math.min(prev + zoomIntensity, 3)); // Max zoom 3x
     } else {
       setScale((prev) => Math.max(prev - zoomIntensity, 1)); // Min zoom 1x
     }
   };
     ...
     <div className="mb-32">
       ...
       <div className="flex w-[90%] mx-auto gap-8">
         ...
         <section className="w-full lg:w-[80%] flex flex-col gap-5">
           ...
           <div className={`${activeSubCategory && selectedProduct?.description ? "" : "gap-5 lg:gap-10"} flex flex-col lg:flex-row items-center `}>
             <div className={`${activeSubCategory && selectedProduct?.description ? "" : "w-full lg:w-[50%]"}`}>
               {/* Category Description */}
               <p className="text-[17px] sm:text-[18px] leading-relaxed text-gray-700 text-justify">{activeSubCategory && selectedProduct?.description ? "" : currentCategory?.content ?? ""}</p>
             </div>

             {/* Image Display */}
             <div className={`${activeSubCategory && selectedProduct?.description ? "" : "w-full lg:w-[50%]"} flex items-center justify-between lg:px-3 lg:py-3`}>
           ...
           {selectedImage && (
             <div onClick={() => setSelectedImage(null)} className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center cursor-zoom-out">
               <img
                 ...
                 className="w-[400px] h-[400px] rounded shadow-xl object-contain cursor-zoom-in"
               />
             </div>
           )}
         </section>
